...
this.prepare_opts();$.extend(this,this.opts);$(this.parent).html(frappe.render_template("listing",this.opts));this.$w=$(this.parent).find('.frappe-list');this.set_events();if(this.page){this.$w.find('.list-toolbar-wrapper').toggle(false);}
if(this.show_filters){this.make_filters();}},add_button:function(label,click,icon){if(this.page){return this.page.add_menu_item(label,click,icon)}else{this.$w.find('.list-toolbar-wrapper').removeClass("hide");$button=$('<button class="btn btn-default"></button>').appendTo(this.$w.find('.list-toolbar')).html((icon?("<i class='"+icon+"'></i> "):"")+label).click(click);return $button}},set_events:function(){var me=this;this.$w.find('.btn-more').click(function(){me.run(true);});this.$w.find(".btn-group-paging .btn").click(function(){me.page_length=cint($(this).attr("data-value"));me.$w.find(".btn-group-paging .btn-info").removeClass("btn-info");$(this).addClass("btn-info");me.run();});if(this.opts.page_length!=20){this.$w.find(".btn-group-paging .btn-info").removeClass("btn-info");this.$w.find(".btn-group-paging .btn[data-value='"+this.opts.page_length+"']").addClass('btn-info');}
if(this.title){this.$w.find('h3').html(this.title).toggle(true);}
this.set_primary_action();if(me.no_toolbar||me.hide_toolbar){me.$w.find('.list-toolbar-wrapper').toggle(false);}},set_primary_action:function(){var me=this;if(this.new_doctype){if(this.listview.settings.set_primary_action){this.listview.settings.set_primary_action(this);}else{this.page.set_primary_action(__("New"),function(){me.make_new_doc(me.new_doctype);},"octicon octicon-plus");}}else{this.page.clear_primary_action();}},make_new_doc:function(doctype){var me=this;frappe.model.with_doctype(doctype,function(){if(me.custom_new_doc){me.custom_new_doc(doctype);}else{if(me.filter_list){frappe.route_options={};$.each(me.filter_list.get_filters(),function(i,f){if(f[2]==="="&&!in_list(frappe.model.std_fields_list,f[1])){frappe.route_options[f[1]]=f[3];}});}
frappe.new_doc(doctype,true);}});},make_filters:function(){this.filter_list=new frappe.ui.FilterList({listobj:this,$parent:this.$w.find('.list-filters').toggle(true),doctype:this.doctype,filter_fields:this.filter_fields});if(frappe.model.is_submittable(this.doctype)){this.filter_list.add_filter(this.doctype,"docstatus","!=",2);};},clear:function(){this.data=[];this.$w.find('.result-list').empty();this.$w.find('.result').toggle(true);this.$w.find('.no-result').toggle(false);this.start=0;},set_filters_from_route_options:function(){var me=this;this.filter_list.clear_filters();$.each(frappe.route_options,function(key,value){var doctype=null;if(key.indexOf(".")!==-1){doctype=key.split(".")[0];key=key.split(".")[1];}
...
different=true;}
if(args.save_list_settings_fields){list_settings.fields=args.fields;};if(different){list_settings.updated_on=moment().toString();}}},set_working:function(flag){this.$w.find('.img-load').toggle(flag);},get_call_args:function(){if(!this.method){var query=this.get_query?this.get_query():this.query;query=this.add_limits(query);var args={query_max:this.query_max,as_dict:1}
...
return args;},render_results:function(r){if(this.start===0)this.clear();this.$w.find('.btn-more, .list-loading').toggle(false);if(r.message){r.values=this.get_values_from_response(r.message);}
...
if(this.onrun)this.onrun();if(this.callback)this.callback(r);this.$w.trigger("render-complete");},get_values_from_response:function(data){if(data.keys&&$.isArray(data.keys)){return frappe.utils.dict(data.keys,data.values);}else{return data;}},render_list:function(values){this.last_page=values;if(this.filter_list){this.filter_values=this.filter_list.get_filters();}
this.render_rows(values);},render_rows:function(values){if(this.meta&&this.meta.image_view){var cols=values.slice();while(cols.length){row=this.add_row(cols[0]);$("<div class='row image-view-marker'></div>").appendTo(row);$(row).addClass('no-hover');this.render_image_view_row(row,cols.splice(0,4),this,i);}
this.render_image_gallery();}else{var m=Math.min(values.length,this.page_length);for(var i=0;i<m;i++){this.render_row(this.add_row(values[i]),values[i],this,i);}}},render_image_gallery:function(){var me=this;frappe.require(["assets/frappe/js/frappe/list/imageview.js","assets/frappe/js/lib/gallery/js/blueimp-gallery.js","assets/frappe/js/lib/gallery/css/blueimp-gallery.css","assets/frappe/js/lib/gallery/js/blueimp-gallery-indicator.js","assets/frappe/js/lib/gallery/css/blueimp-gallery-indicator.css"],function(){me.$w.find(".blueimp-gallery").remove();var gallery=frappe.render_template("blueimp-gallery",{});$(gallery).appendTo(me.$w);me.$w.find(".zoom-view").click(function(event){event.preventDefault();opts={doctype:me.doctype,docname:$(this).parent().attr('data-name'),container:me.$w};new frappe.views.ImageView(opts);});});},update_paging:function(values){if(values.length>=this.page_length){this.$w.find('.btn-more').toggle(true);this.start+=this.page_length;}},add_row:function(row){return $('<div class="list-row">').data("data",(this.meta&&this.meta.image_view)==0?row:null).appendTo(this.$w.find('.result-list')).get(0);},refresh:function(){this.run();},add_limits:function(query){query+=' LIMIT '+this.start+','+(this.page_length+1);return query},set_filter:function(fieldname,label,doctype){if(!doctype)doctype=this.doctype;var filter=this.filter_list.get_filter(fieldname);if(filter){var v=filter.field.get_parsed_value();if(v.indexOf(label)!=-1){return this;}else{if(fieldname=='_user_tags'){this.filter_list.add_filter(doctype,fieldname,'like','%'+label+'%');}else{filter.set_values(doctype,fieldname,'in',v+', '+label);}}}else{if(['_user_tags','_comments','_assign','_liked_by'].indexOf(fieldname)!==-1){this.filter_list.add_filter(doctype,fieldname,'like','%'+label+'%');}else{this.filter_list.add_filter(doctype,fieldname,'=',label);}}
return this;},init_list_settings:function(){if(frappe.model.list_settings[this.doctype]){this.list_settings=frappe.model.list_settings[this.doctype];}else{this.list_settings={};}},});;frappe.get_indicator=function(doc,doctype){if(doc.__unsaved){return[__("Not Saved"),"orange"];}
if(!doctype)doctype=doc.doctype;var settings=frappe.listview_settings[doctype]||{};var is_submittable=frappe.model.is_submittable(doctype),workflow_fieldname=frappe.workflow.get_state_fieldname(doctype);if(doc.docstatus==3){return[__("Queued for saving"),"orange","docstatus,=,3"];}
if(doc.docstatus==4){return[__("Queued for submission"),"orange","docstatus,=,4"];}
if(doc.docstatus==5){return[__("Queued for cancellation"),"orange","docstatus,=,5"];}
if(workflow_fieldname){var value=doc[workflow_fieldname];if(value){var colour={"Success":"green","Warning":"orange","Danger":"red","Primary":"blue",}[locals["Workflow State"][value].style]||"darkgrey";return[__(value),colour,workflow_fieldname+',=,'+value];}}
...
if(doc.status){return[__(doc.status),frappe.utils.guess_colour(doc.status)];}};frappe.ui.FilterList=Class.extend({init:function(opts){$.extend(this,opts);this.filters=[];this.$w=this.$parent;this.set_events();},set_events:function(){var me=this;this.$w.find('.new-filter').bind('click',function(){me.add_filter(me.doctype,'name');});},show_filters:function(){this.$w.find('.show_filters').toggle();if(!this.filters.length){this.add_filter(this.doctype,'name');this.filters[0].$w.find(".filter_field input").focus();}},clear_filters:function(){$.each(this.filters,function(i,f){f.remove(true);});this.filters=[];},add_filter:function(doctype,fieldname,condition,value,hidden){if(doctype&&fieldname&&!frappe.meta.has_field(doctype,fieldname)&&!in_list(frappe.model.std_fields_list,fieldname)){frappe.msgprint({message:__('Filter {0} missing',[fieldname.bold()]),title:'Invalid Filter',indicator:'red'});return;}
this.$w.find('.show_filters').toggle(true);var is_new_filter=arguments.length===0;if(is_new_filter&&this.$w.find(".is-new-filter:visible").length){return;}
var filter=this.push_new_filter(doctype,fieldname,condition,value);if(filter&&is_new_filter){filter.$w.addClass("is-new-filter");}
...
if(df.fieldname=="docstatus"){df.fieldtype="Select",df.options=[{value:0,label:__("Draft")},{value:1,label:__("Submitted")},{value:2,label:__("Cancelled")},{value:3,label:__("Queued for saving")},{value:4,label:__("Queued for submission")},{value:5,label:__("Queued for cancellation")},]}else if(df.fieldtype=='Check'){df.fieldtype='Select';df.options='No\nYes';}else if(['Text','Small Text','Text Editor','Code','Tag','Comments','Dynamic Link','Read Only','Assign'].indexOf(df.fieldtype)!=-1){df.fieldtype='Data';}else if(df.fieldtype=='Link'&&this.$w.find('.condition').val()!="="){df.fieldtype='Data';}
...
return val;},get_condition:function(){return this.$w.find('.condition').val();},freeze:function(){if(this.$btn_group){this.set_filter_button_text();this.$w.toggle(false);return;}
var me=this;this.$btn_group=$('<div class="btn-group">\
   ...
   </button></div>').insertAfter(this.flist.$w.find(".set-filters .new-filter"));this.set_filter_button_text();this.$btn_group.find(".remove-filter").on("click",function(){me.remove();});this.$btn_group.find(".toggle-filter").on("click",function(){me.$w.toggle();})
...
frappe.ui.TagEditor=Class.extend({init:function(opts){$.extend(this,opts);var me=this;this.$w=$('<div class="tag-line">').appendTo(this.parent)
this.$tags=$('<ul>').prependTo(this.$w);this.$tags.tagit({animate:false,allowSpaces:true,placeholderText:__('Add a tag')+"...",onTagAdded:function(ev,tag){if(me.initialized&&!me.refreshing){var tag=tag.find('.tagit-label').text();return frappe.call({method:'frappe.desk.tags.add_tag',args:me.get_args(tag),callback:function(r){var user_tags=me.user_tags.split(",");user_tags.push(tag)
...
this.initialized=true;this.refresh(this.user_tags);this.setup_autocomplete();},setup_autocomplete:function(){var me=this;this.$w.find("input").autocomplete({minLength:0,minChars:0,source:function(request,response){frappe.call({method:"frappe.desk.tags.get_tags",args:{doctype:me.frm.doctype,txt:request.term.toLowerCase()},callback:function(r){response(r.message);}});},});},get_args:function(tag){return{tag:tag,dt:this.frm.doctype,dn:this.frm.docname,}},refresh:function(user_tags){var me=this;if(!me.initialized||me.refreshing)
...
this.refresh();},refresh:function(dirty){if(dirty!==undefined)this.dirty=dirty;this.init_stats();if(this.listview.settings.refresh){this.listview.settings.refresh(this);}
this.set_filters_before_run();if(this.dirty){this.run();}else{if(new Date()-(this.last_updated_on||0)>30000){this.run();}}},set_filters_before_run:function(){var me=this;if(frappe.route_options){this.set_filters_from_route_options();this.dirty=true;}else if(this.list_settings&&this.list_settings.filters&&this.list_settings.updated_on!=this.list_settings_updated_on){this.filter_list.clear_filters();this.list_settings.filters.forEach(function(f){me.filter_list.add_filter(f[0],f[1],f[2],f[3]);});this.dirty=true;}},run:function(more){var me=this;if(this.fresh&&!more){return;}
if(this.listview.settings.before_run){this.listview.settings.before_run(this);}
if(!this.listview.settings.use_route){var route=frappe.get_route();var me=this;if(route[2]){$.each(frappe.utils.get_args_dict_from_url(route[2]),function(key,val){me.set_filter(key,val,true);});}}
this.list_header.find(".list-liked-by-me").toggleClass("text-extra-muted not-liked",!this.is_star_filtered());this.last_updated_on=new Date();this.dirty=false;this.fresh=true;setTimeout(function(){me.fresh=false;},1000);this._super(more);if(this.listview.settings.post_render){this.listview.settings.post_render(this);}
...
__('Make a new {0}',[__(this.doctype)])+'</button></p>'):'';var no_result_message='<div class="msg-box no-border" style="margin: 100px 0px;">\
   <p>'+__("No {0} found",[__(this.doctype)])+'</p>'+new_button+'</div>';return no_result_message;},render_row:function(row,data){data.doctype=this.doctype;this.listview.render(row,data,this);},render_image_view_row:function(row,data){for(var i=0;i<data.length;i++){data[i].doctype=this.doctype;this.listview.render(row,data[i],this)}},get_args:function(){var args={doctype:this.doctype,fields:this.listview.fields,filters:this.filter_list.get_filters(),order_by:this.listview.order_by||undefined,group_by:this.listview.group_by||undefined,with_comment_count:true}
...
if(docname.length>=1){me.dialog=frappe.ui.to_do_dialog({obj:me,method:'frappe.desk.form.assign_to.add_multiple',doctype:me.doctype,docname:docname,bulk_assign:true,re_assign:true,callback:function(){me.dirty=true;me.refresh();}});me.dialog.clear();me.dialog.show();}
...
else{frappe.msgprint(__("Select records for assignment"))}},true);},init_like:function(){var me=this;this.$page.find(".result-list").on("click",".like-action",frappe.ui.click_toggle_like);this.list_header.find(".list-liked-by-me").on("click",function(){if(me.is_star_filtered()){me.remove_liked_by_me();}else{me.liked_by_me();}});if(!frappe.dom.is_touchscreen()){frappe.ui.setup_like_popover(this.$page.find(".result-list"),".liked-by");}},init_select_all:function(){var me=this;if(this.can_delete||this.listview.settings.selectable){this.list_header.find(".list-select-all").on("click",function(){me.$page.find('.list-delete').prop("checked",$(this).prop("checked"));me.toggle_delete();});this.$page.on("click",".list-delete",function(event){me.toggle_delete();var $this=$(this);if(event.shiftKey&&$this.prop("checked")){var $end_row=$this.parents(".list-row");var $start_row=$end_row.prevAll(".list-row").find(".list-delete:checked").last().parents(".list-row");if($start_row){$start_row.nextUntil($end_row).find(".list-delete").prop("checked",true);}}});this.$w.on("render-complete",function(){me.toggle_delete();});}},toggle_delete:function(){var me=this;if(this.$page.find(".list-delete:checked").length){this.page.set_primary_action(__("Delete"),function(){me.delete_items()},"octicon octicon-trashcan");this.page.btn_primary.addClass("btn-danger");}else{this.page.btn_primary.removeClass("btn-danger");this.set_primary_action();}},toggle_tags:function(){if(this.tags_shown){$(".tag-row").addClass("hide");this.tags_shown=false;}else{$(".tag-row").removeClass("hide");this.tags_shown=true;}},get_checked_items:function(){var me=this;return $.map(this.$page.find('.list-delete:checked'),function(e){if(me.meta.image_view==0){return $(e).parents(".list-row:first").data('data');}
...
return;frappe.confirm(__('Delete permanently?'),function(){me.set_working(true);return frappe.call({method:'frappe.desk.reportview.delete_items',freeze:true,args:{items:$.map(dl,function(d,i){return d.name}),doctype:me.doctype},callback:function(){me.$page.find('.list-select-all').prop("checked",false);frappe.utils.play_sound("delete");me.set_working(false);me.dirty=true;me.refresh();}})});},init_stats:function(){var me=this;this.sidebar_stats=new frappe.views.ListSidebar({doctype:this.doctype,stats:this.listview.stats,parent:this.$page.find('.layout-side-section'),set_filter:function(fieldname,label){me.set_filter(fieldname,label);},page:this.page,doclistview:this})},set_filter:function(fieldname,label,no_run){var filter=this.filter_list.get_filter(fieldname);if(filter){var v=cstr(filter.field.get_parsed_value());if(v.indexOf(label)!=-1){return false;}else{if(fieldname=='_user_tags'||fieldname=="_liked_by"){this.filter_list.add_filter(this.doctype,fieldname,'like','%'+label);}else{filter.set_values(this.doctype,fieldname,'in',v+', '+label);}}}else{if(fieldname=='_user_tags'||fieldname=="_liked_by"){this.filter_list.add_filter(this.doctype,fieldname,'like','%'+label);}else{this.filter_list.add_filter(this.doctype,fieldname,'=',label);}}
if(!no_run)
this.run();},call_for_selected_items:function(method,args){var me=this;args.names=$.map(this.get_checked_items(),function(d){return d.name;});frappe.call({method:method,args:args,freeze:true,callback:function(r){if(!r.exc){me.list_header.find(".list-select-all").prop("checked",false);me.refresh();}}});}});;frappe.provide('frappe.views');frappe.views.ListSidebar=Class.extend({init:function(opts){$.extend(this,opts);this.make();this.get_stats();},make:function(){var sidebar_content=frappe.render_template("list_sidebar",{doctype:this.doclistview.doctype});this.sidebar=$('<div class="list-sidebar overlay-sidebar hidden-xs hidden-sm"></div>').html(sidebar_content).appendTo(this.page.sidebar.empty());this.setup_reports();this.setup_assigned_to_me();this.setup_list_view_switching();if(frappe.views.calendar[this.doctype]){this.sidebar.find(".calendar-link, .gantt-link").removeClass("hide");}
if(frappe.treeview_settings[this.doctype]){this.sidebar.find(".tree-link").removeClass("hide");}},setup_reports:function(){var me=this;var added=[];var dropdown=this.page.sidebar.find('.reports-dropdown');var divider=false;var add_reports=function(reports){$.each(reports,function(name,r){if(!r.ref_doctype||r.ref_doctype==me.doctype){var report_type=r.report_type==='Report Builder'?'Report/'+r.ref_doctype:'query-report';var route=r.route||report_type+'/'+r.name;if(added.indexOf(route)===-1){added.push(route);if(!divider){$('<li role="separator" class="divider"></li>').appendTo(dropdown);divider=true;}
$('<li><a href="#'+route+'">'
...
return listview;}
frappe.views.ListView=Class.extend({init:function(doclistview,doctype){this.doclistview=doclistview;this.doctype=doctype;this.meta=frappe.get_doc("DocType",this.doctype);this.settings=frappe.listview_settings[this.doctype]||{};if(this.meta.__listview_template){this.template_name=doctype+"_listview";frappe.templates[this.template_name]=this.meta.__listview_template;}
...
this.columns.push(name_column);this.total_colspans=this.columns[0].colspan;if(frappe.model.is_submittable(this.doctype)||this.settings.get_indicator||this.workflow_state_fieldname){this.columns.push({colspan:this.settings.colwidths&&this.settings.colwidths.indicator||3,type:"Indicator",title:"Status"});this.total_colspans+=this.columns[1].colspan;}
...
if(this.meta&&this.meta.image_view==0){this.render_list_row(row,data);}
else{this.render_list_image(row,data);}
if(this.settings.post_render_item){this.settings.post_render_item(this,row,data);}
this.render_tags(row,data);},render_list_row:function(row,data){var main=frappe.render_template("list_item_main",{data:data,columns:this.columns,subject:this.get_avatar_and_id(data,true),list:this,right_column:this.settings.right_column});$(frappe.render_template("list_item_row",{data:data,main:main,list:this,right_column:this.settings.right_column})).appendTo(row);},render_list_image:function(row,data){this.allowed_type=["Check","Currency","Data","Date","Datetime","Float","Int","Link","Percent","Select","Read Only","Time"];img_col=$(frappe.render_template("image_view_item_row",{data:data,list:this,columns:this.columns,allowed_type:this.allowed_type,item_image:data.image?"url('"+data.image+"')":null,color:frappe.get_palette(data.item_name),subject:this.get_avatar_and_id(data,true),right_column:this.settings.right_column})).data("data",data).appendTo($(row).find(".image-view-marker"));},render_tags:function(row,data){var me=this;var row2=$('<div class="tag-row">\
   <div class="list-tag xs-hidden"></div>\
   <div class="clearfix"></div>\
  </div>').appendTo(row);if(!me.doclistview.tags_shown){row2.addClass("hide");}
var tag_editor=new frappe.ui.TagEditor({parent:row2.find(".list-tag"),frm:{doctype:this.doctype,docname:data.name},user_tags:data._user_tags,on_change:function(user_tags){data._user_tags=user_tags;}});tag_editor.$w.on("click",".tagit-label",function(){me.doclistview.set_filter("_user_tags",$(this).text());});},get_avatar_and_id:function(data,without_workflow){data._without_workflow=without_workflow;data.css_seen='';if(data._seen){var seen=JSON.parse(data._seen);if(seen&&seen.indexOf(frappe.session.user)!==-1){data.css_seen='seen'}}
...
args.doctype=event.doctype||this.doctype;return{args:args,field_map:this.field_map};},fix_end_date_for_event_render:function(event){if(event.allDay){event.start=event.start?$.fullCalendar.moment(event.start).stripTime():null;event.end=event.end?$.fullCalendar.moment(event.end).add(1,"day").stripTime():null;}}});frappe.provide("frappe.views.calendar");frappe.views.GanttFactory=frappe.views.Factory.extend({make:function(route){var me=this;me.doctype=route[1];frappe.require(['assets/frappe/js/lib/jQuery.Gantt/css/style.css','assets/frappe/js/lib/jQuery.Gantt/js/jquery.fn.gantt.js'],function(){frappe.model.with_doctype(me.doctype,function(){var page=me.make_page();$(page).on("show",function(){page.ganttview.set_filters_from_route_options();});var options={doctype:me.doctype,parent:page};$.extend(options,frappe.views.calendar[me.doctype]||{});page.ganttview=new frappe.views.Gantt(options);});});}});frappe.views.Gantt=frappe.views.CalendarBase.extend({init:function(opts){$.extend(this,opts);this.make_page();frappe.route_options?this.set_filters_from_route_options():this.refresh();},make_page:function(){var module=locals.DocType[this.doctype].module,me=this;this.page=this.parent.page;this.page.set_title(__("Gantt Chart")+" - "+__(this.doctype));frappe.breadcrumbs.add(module,this.doctype);this.page.set_secondary_action(__("Refresh"),function(){me.refresh();},"icon-refresh")
this.page.add_field({fieldtype:"Date",label:"From",fieldname:"start","default":frappe.datetime.month_start(),change:function(){me.refresh();},input_css:{"z-index":3}});this.page.add_field({fieldtype:"Date",label:"To",fieldname:"end","default":frappe.datetime.month_end(),change:function(){me.refresh();},input_css:{"z-index":3}});this.add_filters();this.wrapper=$("<div style='position:relative;z-index:1;'></div>").appendTo(this.page.main);},refresh:function(){var me=this;return frappe.call({method:this.get_events_method,type:"GET",args:{doctype:this.doctype,start:this.page.fields_dict.start.get_parsed_value(),end:this.page.fields_dict.end.get_parsed_value(),filters:this.get_filters()},callback:function(r){$(me.wrapper).empty();if(!r.message||!r.message.length){$(me.wrapper).html('<p class="text-muted" style="padding: 15px;">'+__('Nothing to show for this selection')+'</p>');}else{var gantt_area=$('<div class="gantt">').appendTo(me.wrapper);gantt_area.gantt({source:me.get_source(r),navigate:"scroll",scale:me.gantt_scale||"days",minScale:"hours",maxScale:"months",itemsPerPage:20,onItemClick:function(data){frappe.set_route('Form',me.doctype,data.name);},onAddClick:function(dt,rowId){frappe.new_doc(me.doctype,true);}});}}})},get_source:function(r){var source=[],me=this;$.each(r.message,function(i,v){v["title"]=v[me.field_map["title"]];v.desc=v.title
+(v.name?("<br>"+v.name):"");$.each(v,function(key,value){if(!in_list(["name","title",me.field_map["title"],"desc"],key)&&value){var label=frappe.meta.get_label(me.doctype,key);if(label){v.desc+="<br>"+label+": "+value;}}});$.each(me.field_map,function(target,source){v[target]=v[source];});if(v.start&&!v.end){v.end=new Date(v.start)
v.end.setHours(v.end.getHours()+1);}
if(me.style_map){v.cssClass=me.style_map[v.status]}else if(me.get_css_class){v.cssClass=me.get_css_class(v);}else{v.cssClass=frappe.utils.guess_style(v.status,"standard")}
if(v.start&&v.end){source.push({name:v.title,desc:v.status,values:[{name:v.title,desc:v.desc,from:'/Date('+moment(v.start).format("X")+'000)/',to:'/Date('+moment(v.end).format("X")+'000)/',customClass:{'danger':'ganttRed','warning':'ganttOrange','info':'ganttBlue','success':'ganttGreen','':'ganttGray'}[v.cssClass],dataObj:v}]})}});return source}});;frappe.templates["blueimp-gallery"] = '<div id="blueimp-gallery" class="blueimp-gallery blueimp-gallery-controls"> <div class="slides"></div> <h3 class="title"></h3> <a class="prev">‹</a> <a class="next">›</a> <a class="close">×</a> <a class="play-pause"></a> <ol class="indicator"></ol> </div>';
...
frappe.views.TreeFactory=frappe.views.Factory.extend({make:function(route){frappe.model.with_doctype(route[1],function(){var options={doctype:route[1]};if(!frappe.treeview_settings[route[1]]&&!frappe.meta.get_docfield(route[1],"is_group")){msgprint(__("Tree view not available for {0}",[route[1]]));return false;}
...
this.tree=new frappe.ui.Tree({parent:$(me.parent).find(".layout-main-section"),label:me.args[me.opts.root_label]||me.opts.root_label||me.root,args:me.args,method:me.get_tree_nodes,toolbar:me.get_toolbar(),get_label:me.opts.get_label,onrender:me.opts.onrender});},get_toolbar:function(){var me=this;if(this.opts.toolbar){return this.opts.toolbar;}else{return[{toggle_btn:true},{label:__("Edit"),condition:function(node){return!node.root&&me.can_read;},click:function(node){frappe.set_route("Form",me.doctype,node.label);}},{label:__("Add Child"),condition:function(node){return me.can_create&&node.expandable;},click:function(node){me.new_node();},btnClass:"hidden-xs"},{label:__("Rename"),condition:function(node){return!node.root&&me.can_write;},click:function(node){frappe.model.rename_doc(me.doctype,node.label,function(new_name){node.$a.html(new_name);});},btnClass:"hidden-xs"},{label:__("Delete"),condition:function(node){return!node.root&&me.can_delete;},click:function(node){frappe.model.delete_doc(me.doctype,node.label,function(){node.parent.remove();});},btnClass:"hidden-xs"}]}},new_node:function(){var me=this;var node=me.tree.get_selected_node();if(!(node&&node.expandable)){frappe.msgprint(__("Select a group node first."));return;}
this.prepare_fields()
var d=new frappe.ui.Dialog({title:__('New {0}',[__(me.doctype)]),fields:me.fields})
d.set_value("is_group",0);d.set_primary_action(__("Create New"),function(){var btn=this;var v=d.get_values();if(!v)return;var node=me.tree.get_selected_node();v.parent=node.label;v.doctype=me.doctype;if(node.root){v.is_root=1;v.parent_account=null;}else{v.is_root=0;v.root_type=null;}
$.extend(me.args,v)
...
node.reload();}}});});d.show();},prepare_fields:function(){var me=this;this.fields=[{fieldtype:'Data',fieldname:'name_field',label:__('New {0} Name',[__(me.doctype)]),reqd:true},{fieldtype:'Check',fieldname:'is_group',label:__('Group Node'),description:__("Further nodes can be only created under 'Group' type nodes")}]
if(me.opts.fields){me.fields=me.opts.fields;}},set_primary_action:function(){var me=this;if(!this.opts.disable_add_node){me.page.set_primary_action(__("New"),function(){me.new_node();},"octicon octicon-plus")}},set_menu_item:function(){var me=this;this.menu_items=[{label:__('View List'),action:function(){frappe.set_route('List',me.doctype);}},{label:__('Refresh'),action:function(){me.make_tree();}},]
...
if(has_perm){me.page.add_menu_item(menu_item["label"],menu_item["action"]);}})}});;