...
me.parent.reportview.run();});},make_report_view:function(){this.page.set_title(__(this.doctype));var module=locals.DocType[this.doctype].module;frappe.breadcrumbs.add(module,this.doctype);if(frappe.model.can_write(this.doctype)){this.page.footer.html('<p class="text-muted">'+__("Tip: Double click cell to edit")+'</p>').removeClass("hide");}
this.parent.reportview=new frappe.views.ReportView({doctype:this.doctype,docname:this.docname,parent:this.parent});}});frappe.views.ReportView=frappe.ui.Listing.extend({init:function(opts){var me=this;$.extend(this,opts);this.can_delete=frappe.model.can_delete(me.doctype);this.tab_name='`tab'+this.doctype+'`';this.setup();},set_init_columns:function(){var columns=[['name'],['owner']];$.each(frappe.meta.docfield_list[this.doctype],function(i,df){if(df.in_filter&&df.fieldname!='naming_series'&&df.fieldtype!='Table'){columns.push([df.fieldname]);}});this.columns=columns;},setup:function(){var me=this;this.page=this.parent.page;this.page_title=__('Report')+': '+__(this.docname?(this.doctype+' - '+this.docname):this.doctype);this.page.set_title(this.page_title);this.init_list_settings();this.make({page:this.parent.page,method:'frappe.desk.reportview.get',save_list_settings:true,get_args:this.get_args,parent:this.page.main,start:0,show_filters:true,allow_delete:true,});this.make_new_and_refresh();this.make_delete();this.make_column_picker();this.make_sorter();this.setup_print();this.make_export();this.set_init_columns();this.make_save();this.make_user_permissions();this.set_tag_and_status_filter();this.page.add_menu_item(__("Add to Desktop"),function(){frappe.add_to_desktop(__('{0} Report',[me.doctype]),me.doctype);},true);},make_new_and_refresh:function(){var me=this;this.page.set_primary_action(__("Refresh"),function(){me.run();});this.page.add_menu_item(__("New {0}",[this.doctype]),function(){me.make_new_doc(me.doctype);},true);},set_init_columns:function(){var me=this;var columns=[];if(this.list_settings.fields&&!this.docname){this.list_settings.fields.forEach(function(field){var coldef=me.get_column_info_from_field(field);if(!in_list(['_seen','_comments','_user_tags','_assign','_liked_by','docstatus'],coldef[0])){columns.push(coldef);}});};if(!columns.length){var columns=[['name',this.doctype],];$.each(frappe.meta.docfield_list[this.doctype],function(i,df){if((df.in_filter||df.in_list_view)&&df.fieldname!='naming_series'&&!in_list(frappe.model.no_value_type,df.fieldtype)){columns.push([df.fieldname,df.parent]);}});}
this.columns=columns;},set_columns_and_filters:function(opts){var me=this;this.filter_list.clear_filters();if(opts.columns){this.columns=opts.columns;}
...
if(opts.sort_by)this.sort_by_select.val(opts.sort_by);if(opts.sort_order)this.sort_order_select.val(opts.sort_order);if(opts.sort_by_next)this.sort_by_next_select.val(opts.sort_by_next);if(opts.sort_order_next)this.sort_order_next_select.val(opts.sort_order_next);},set_route_filters:function(first_load){var me=this;if(frappe.route_options){this.set_filters_from_route_options();return true;}else if(this.list_settings&&this.list_settings.filters&&!this.docname&&(this.list_settings.updated_on!=this.list_settings_updated_on)){this.filter_list.clear_filters();$.each(this.list_settings.filters,function(i,f){me.filter_list.add_filter(f[0],f[1],f[2],f[3]);});return true;}
this.list_settings_updated_on=this.list_settings.updated_on;},setup_print:function(){var me=this;this.page.add_menu_item(__("Print"),function(){var title=__(me.docname||me.doctype);frappe.render_grid({grid:me.grid,title:title});},true);},get_args:function(){var me=this;return{doctype:this.doctype,fields:$.map(this.columns,function(v){return me.get_full_column_name(v)}),order_by:this.get_order_by(),filters:this.filter_list.get_filters(),save_list_settings_fields:1,with_childnames:1}},get_order_by:function(){var order_by=[];var sort_by_select=this.get_selected_table_and_column(this.sort_by_select);if(sort_by_select){order_by.push(sort_by_select+" "+this.sort_order_select.val());}
...
docfield=columnDef.report_docfield;docfield.link_onclick=repl('frappe.container.page.reportview.set_filter("%(fieldname)s", "%(value)s").run()',{fieldname:docfield.fieldname,value:value});}
...
this.col_defs=columns;this.dataView=new Slick.Data.DataView();this.set_data(this.data);var grid_wrapper=this.$w.find('.result-list').addClass("slick-wrapper");if(!options.autoHeight)
grid_wrapper.css('height','500px');this.grid=new Slick.Grid(grid_wrapper.get(0),this.dataView,columns,options);if(!frappe.dom.is_touchscreen()){this.grid.setSelectionModel(new Slick.CellSelectionModel());this.grid.registerPlugin(new Slick.CellExternalCopyManager({dataItemColumnValueExtractor:function(item,columnDef,value){return item[columnDef.field];}}));}
frappe.slickgrid_tools.add_property_setter_on_resize(this.grid);if(this.start!=0&&!options.autoHeight){this.grid.scrollRowIntoView(this.data.length-1);}
...
this.clear();this.column_list=$(this.dialog.body).find('.column-list');$.each(columns,function(i,c){me.add_column(c);});new Sortable(this.column_list.get(0),{onUpdate:function(event){me.columns=[];$.each($(me.dialog.body).find('.column-list .column-list-item'),function(i,ele){me.columns.push($(ele).data("fieldselect"))});}});$(this.dialog.body).find('.btn-add').click(function(){me.add_column(['name']);});this.dialog.show();},add_column:function(c){if(!c)return;var me=this;var w=$('<div class="column-list-item">\
    <i class="icon icon-sort text-muted drag-handle"></i>\
    <a class="close">&times;</a>\
   </div>').appendTo(this.column_list);var fieldselect=new frappe.ui.FieldSelect({parent:w,doctype:this.doctype});fieldselect.val((c[1]||this.doctype)+"."+c[0]);w.data("fieldselect",fieldselect);w.find('.close').data("fieldselect",fieldselect).click(function(){delete me.columns[me.columns.indexOf($(this).data('fieldselect'))];$(this).parent().remove();});this.columns.push(fieldselect);},update_column_selection:function(){this.dialog.hide();var columns=$.map(this.columns,function(v){return(v&&v.selected_fieldname&&v.selected_doctype)?[[v.selected_fieldname,v.selected_doctype]]:null;});this.list.columns=columns;this.list.run();}});;frappe.provide("frappe.views");frappe.provide("frappe.query_reports");frappe.standard_pages["query-report"]=function(){var wrapper=frappe.container.add_page('query-report');frappe.ui.make_app_page({parent:wrapper,title:__('Query Report'),single_column:true,});frappe.query_report=new frappe.views.QueryReport({parent:wrapper,});$(wrapper).bind("show",function(){frappe.query_report.load();});}
frappe.views.QueryReport=Class.extend({init:function(opts){$.extend(this,opts);this.page=this.parent.page;this.parent.query_report=this;this.make();},slickgrid_options:{enableColumnReorder:false,showHeaderRow:true,headerRowHeight:30,explicitInitialization:true,multiColumnSort:true},make:function(){this.wrapper=$("<div>").appendTo(this.page.main);$('<div class="waiting-area" style="display: none;"></div>\
  ...
  </div>').appendTo(this.wrapper);this.chart_area=this.wrapper.find(".chart_area");this.make_toolbar();},make_toolbar:function(){var me=this;this.page.set_secondary_action(__('Refresh'),function(){me.refresh();});this.page.add_menu_item(__('Edit'),function(){if(!frappe.user.is_report_manager()){msgprint(__("You are not allowed to create / edit reports"));return false;}
frappe.set_route("Form","Report",me.report_name);},true);this.page.add_menu_item(__("Print"),function(){me.print_report();},true);this.page.add_menu_item(__("PDF"),function(){me.pdf_report();},true);this.page.add_menu_item(__('Export'),function(){me.export_report();},true);if(frappe.model.can_set_user_permissions("Report")){this.page.add_menu_item(__("User Permissions"),function(){frappe.route_options={doctype:"Report",name:me.report_name};frappe.set_route("user-permissions");},true);}
...
this.wrapper.find(".no-report-area").html(msg).toggle(true);}},setup_report:function(){var me=this;this.page.set_title(__(this.report_name));this.page.clear_inner_toolbar();this.setup_filters();this.chart_area.toggle(false);var report_settings=frappe.query_reports[this.report_name];$.when(function(){if(report_settings.onload){return report_settings.onload(me);}}()).then(function(){me.refresh();})},print_report:function(){if(!frappe.model.can_print(this.report_doc.ref_doctype)){msgprint(__("You are not allowed to print this report"));return false;}
if(this.html_format){var content=frappe.render(this.html_format,{data:this.dataView.getItems(),filters:this.get_values(),report:this});frappe.render_grid({content:content,title:__(this.report_name)});}else{frappe.render_grid({grid:this.grid,report:this,title:__(this.report_name)});}},pdf_report:function(){if(!frappe.model.can_print(this.report_doc.ref_doctype)){msgprint(__("You are not allowed to make PDF for this report"));return false;}
if(this.html_format){var content=frappe.render(this.html_format,{data:this.dataView.getItems(),filters:this.get_values(),report:this});var html=frappe.render_template("print_template",{content:content,title:__(this.report_name)});}else{var columns=this.grid.getColumns();var data=this.grid.getData().getItems();var content=frappe.render_template("print_grid",{columns:columns,data:data,title:__(this.report_name)})
var html=frappe.render_template("print_template",{content:content,title:__(this.report_name)});}
var formData=new FormData();formData.append("html",html);var blob=new Blob([],{type:"text/xml"});formData.append("blob",blob);var xhr=new XMLHttpRequest();xhr.open("POST",'/api/method/frappe.utils.print_format.report_to_pdf');xhr.setRequestHeader("X-Frappe-CSRF-Token",frappe.csrf_token);xhr.responseType="arraybuffer";xhr.onload=function(success){if(this.status===200){var blob=new Blob([success.currentTarget.response],{type:"application/pdf"});var objectUrl=URL.createObjectURL(blob);window.open(objectUrl);}};xhr.send(formData);},setup_filters:function(){this.clear_filters();var me=this;$.each(frappe.query_reports[this.report_name].filters||[],function(i,df){if(df.fieldtype==="Break"){me.page.add_break();}else{var f=me.page.add_field(df);$(f.wrapper).addClass("filters pull-left");me.filters.push(f);if(df["default"]){f.set_input(df["default"]);}
...
f.set_mandatory&&f.set_mandatory(f.$input.val());});}});var $filters=$(this.parent).find('.page-form .filters');$(this.parent).find('.page-form').toggle($filters.length?true:false);this.set_route_filters()
this.set_filters_by_name();},clear_filters:function(){this.filters=[];$(this.parent).find('.page-form .filters').remove();},set_route_filters:function(){var me=this;if(frappe.route_options){$.each(this.filters||[],function(i,f){if(frappe.route_options[f.df.fieldname]!=null){f.set_input(frappe.route_options[f.df.fieldname]);}});}
...
if(raise&&mandatory_fields.length){this.wrapper.find(".waiting-area").empty().toggle(false);this.wrapper.find(".no-report-area").html(__("Please set filters")).toggle(true);if(raise){console.log('filter missing: '+mandatory_fields);throw"Filters required";}}
...
this.set_message(res.message);this.setup_chart(res);},make_columns:function(columns){var me=this;var formatter=this.get_formatter();this.columns=[{id:"_id",field:"_id",name:__("Sr No"),width:60}].concat($.map(columns,function(c,i){if($.isPlainObject(c)){var df=c;}else if(c.indexOf(":")!==-1){var opts=c.split(":");var df={label:opts.length<=2?opts[0]:opts.slice(0,opts.length-2).join(":"),fieldtype:opts.length<=2?opts[1]:opts[opts.length-2],width:opts.length<=2?opts[2]:opts[opts.length-1]};if(df.fieldtype.indexOf("/")!==-1){var tmp=df.fieldtype.split("/");df.fieldtype=tmp[0];df.options=tmp[1];}
...
if(initial_depth&&item.indent&&item.indent>=(initial_depth-1)){item._collapsed=true;}}},tree_filter:function(item){var me=frappe.query_report;if(!me.inline_filter(item))return false;try{var parent_name=item[me.parent_field];while(parent_name){if(!me.item_by_name[parent_name]||me.item_by_name[parent_name]._collapsed){return false;}
...
me.filter_inputs[i].val(v);})},make_filters:function(){var me=this;$.each(this.filters,function(i,v){v.fieldname=v.fieldname||v.label.replace(/ /g,'_').toLowerCase();var input=null;if(v.fieldtype=='Select'){input=me.page.add_select(v.label,v.options||[v.default_value]);}else if(v.fieldtype=="Link"){input=me.page.add_data(v.label);input.autocomplete({source:v.list||[],});}else if(v.fieldtype==='Button'&&v.label===__("Refresh")){input=me.page.set_primary_action(v.label,null,v.icon);}else if(v.fieldtype==='Button'){input=me.page.add_menu_item(v.label,null,true);}else if(v.fieldtype==='Date'){input=me.page.add_date(v.label);}else if(v.fieldtype==='Label'){input=me.page.add_label(v.label);}else if(v.fieldtype==='Data'){input=me.page.add_data(v.label);}else if(v.fieldtype==='Check'){input=me.page.add_check(v.label);}
...
this.make();this.show_zero=$('.show-zero input:checked').length;this.load_filter_values();this.setup_columns();this.setup_dataview_columns();this.apply_link_formatters();this.prepare_data();this.round_off_data();this.prepare_data_view();show_alert("Updated",2);this.render();this.setup_chart&&this.setup_chart();},setup_dataview_columns:function(){this.dataview_columns=$.map(this.columns,function(col){return!col.hidden?col:null;});},make:function(){var me=this;this.chart_area=$('<div class="chart"></div>').appendTo(this.wrapper);this.page.add_menu_item(__("Export"),function(){return me.export();},true);this.grid_wrapper=$("<div style='height: 500px; border: 1px solid #aaa; \
   background-color: #eee; margin-top: 15px;'>").appendTo(this.wrapper);this.id=frappe.dom.set_unique_id(this.grid_wrapper.get(0));$('<div class="checkbox show-zero">\
...
var chart_data=this.get_chart_data?this.get_chart_data():null;this.chart=new frappe.ui.Chart({wrapper:this.chart_area,data:chart_data});},setup_chart_check:function(){var me=this;me.wrapper.bind('make',function(){me.wrapper.on("click",".chart-check",function(){var checked=$(this).prop("checked");var id=$(this).attr("data-id");if(me.item_by_name){if(me.item_by_name[id]){me.item_by_name[id].checked=checked?true:false;}}else{$.each(me.data,function(i,d){if(d.id==id)d.checked=checked;});}
...
return false;},});;frappe.templates["print_grid"] = ' {% if(title) { %} <h2>{%= __(title) %}</h2> <hr> {% } %} <table class="table table-bordered">  <thead> <tr> {% var columns_length = columns.length; for (var i=0; i < columns_length; i++) { var col = columns[i]; %} {% if(col.name && col._id !== "_check") { %} <th style="min-width: {%= col.minWidth %}px" {% if(col.docfield && in_list(["Float", "Currency", "Int"], col.docfield.fieldtype)) { %} class="text-right" {% } %}>{%= __(col.name) %}</th> {% } %} {% } %} </tr> </thead>  <tbody> {% for (var i=0, l= data.length; i < l; i++) { var row = data[i]; %} <tr> {% for(var c=0; c < columns_length; c++) { var col = columns[c];%} {% if(col.name && col._id !== "_check") { %} {% var value = col.fieldname ? row[col.fieldname] : row[col.id]; %} <td>{%= col.formatter ? col.formatter(i, c, value, col, row, true) : value %}</td> {% } %} {% } %} </tr> {% } %} </tbody> </table> ';
