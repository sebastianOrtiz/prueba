...
bsEditor=Class.extend({init:function(options){this.options=$.extend({},this.default_options,options||{});this.edit_mode=true;if(this.options.editor){this.setup_editor(this.options.editor);this.setup_fixed_toolbar();}else if(this.options.parent){this.wrapper=$("<div></div>").appendTo(this.options.parent);this.setup_editor($("<div class='frappe-list'></div>").appendTo(this.wrapper));this.setup_inline_toolbar();this.editor.addClass("text-editor");this.set_editing();}},setup_editor:function(editor){var me=this;this.editor=$(editor);this.editor.on("click",function(){if(me.edit_mode&&!me.editing){me.set_editing();}}).on("mouseup keyup mouseout",function(){var html=me.clean_html();if(me.editing){me.toolbar.save_selection();me.toolbar.update();if(html!=me.last_html){me.options.change&&me.options.change(html);me.last_html=html;}}}).data("object",this);this.bind_hotkeys();this.init_file_drops();},set_editing:function(){this.editor.attr('contenteditable',true);this.toolbar.show();if(this.options.editor)
...
this.toolbar=window.bs_editor_toolbar;},setup_inline_toolbar:function(){this.toolbar=new bsEditorToolbar(this.options,this.wrapper,this.editor);},onhide:function(){this.editing=false;this.options.onsave&&this.options.onsave(this);this.options.change&&this.options.change(this.get_value());},toggle_edit_mode:function(bool){this.edit_mode=bool;if(this.edit_mode){this.editor.trigger("click");}},default_options:{hotKeys:{'ctrl+b meta+b':'bold','ctrl+i meta+i':'italic','ctrl+u meta+u':'underline','ctrl+z meta+z':'undo','ctrl+y meta+y meta+shift+z':'redo','ctrl+l meta+l':'justifyleft','ctrl+e meta+e':'justifycenter','ctrl+j meta+j':'justifyfull','shift+tab':'outdent','tab':'indent'},toolbar_selector:'[data-role=editor-toolbar]',command_role:'edit',selection_marker:'edit-focus-marker',selection_color:'darkgrey',remove_typography:false,max_file_size:5,},bind_hotkeys:function(){var me=this;$.each(this.options.hotKeys,function(hotkey,command){me.editor.keydown(hotkey,function(e){if(me.editor.attr('contenteditable')&&me.editor.is(':visible')){e.preventDefault();e.stopPropagation();me.toolbar.execCommand(command);return false;}}).keyup(hotkey,function(e){if(me.editor.attr('contenteditable')&&me.editor.is(':visible')){e.preventDefault();e.stopPropagation();return false;}});});},clean_html:function(){var html=this.editor.html()||"";if(!$.trim(this.editor.text())&&!(this.editor.find("img")))html="";if(this.options.remove_typography){var tmp=$("<div></div>").html(html);tmp.find("*").removeAttr("style").removeAttr("font");html=tmp.html();}
...
return;if(value==null)value="";this.last_html=value;this.editor.html(value);}})
bsEditorToolbar=Class.extend({init:function(options,parent,editor){this.options=options;this.editor=editor;this.inline=!!parent;this.options.toolbar_style=$.extend((this.inline?this.inline_style:this.fixed_style),this.options.toolbar_style||{});this.make(parent);this.toolbar.css(this.options.toolbar_style);this.setup_image_button();this.bind_events();},fixed_style:{position:"fixed",top:"0px","padding-top":"5px",width:"100%",height:"45px","background-color":"black",display:"none"},inline_style:{"padding-top":"5px",},make:function(parent){if(!parent)
parent=$("body");if(!parent.find(".frappe-list-toolbar").length){this.toolbar=$(frappe.render_template("editor")).prependTo(parent);if(this.inline){this.toolbar.find("[data-action]").remove();}else{this.toolbar.find(".btn-toolbar").addClass("container");}}},setup_image_button:function(){var me=this;this.file_input=this.toolbar.find('input[type="file"]').css({'visibility':'hidden','width':0,'height':0});this.toolbar.find(".btn-insert-img").on("click",function(){me.file_input.trigger("click");})},show:function(){var me=this;this.toolbar.toggle(true);if(!this.inline){$("body").animate({"padding-top":this.toolbar.outerHeight()},{complete:function(){me.toolbar.css("z-index",1001);}});}},hide:function(){if(!this.editor)
...
this.editor&&this.editor.attr('contenteditable',false).data("object").onhide();this.editor=null;},bind_events:function(){var me=this;this.toolbar.find('a[data-'+me.options.command_role+']').click(function(e){me.restore_selection();me.editor.focus();me.execCommand($(this).data(me.options.command_role));me.save_selection();if(me.toolbar.find("ul.dropdown-menu:visible").length)
me.toolbar.find('[data-toggle="dropdown"]').dropdown("toggle");e.stopPropagation();e.preventDefault();return false;});this.toolbar.find('[data-toggle=dropdown]').click(function(){me.restore_selection()});this.toolbar.find(".btn-add-link").on("click",function(){if(!me.toolbar.bs_link_editor){if(me.inline){me.toolbar.bs_link_editor=new bsLinkEditor(me);}else{if(!window.bs_link_editor){window.bs_link_editor=new bsLinkEditor(me);}
...
me.save_selection();this.value='';return false;});this.toolbar.find("[data-action='Save']").on("click",function(){me.hide();});this.toolbar.find(".btn-html").on("click",function(){new bsHTMLEditor().show(me.editor);});},update:function(){var me=this;if(this.toolbar){$(this.toolbar).find('.btn[data-'+this.options.command_role+']').each(function(){var command=$(this).data(me.options.command_role);try{var query_command_state=document.queryCommandState(command);}catch(e){var query_command_state=false;}
if(query_command_state){$(this).addClass(me.options.active_toolbar_class);}else{$(this).removeClass(me.options.active_toolbar_class);}});}},execCommand:function(commandWithArgs,valueArg){var commandArr=commandWithArgs.split(' '),command=commandArr.shift(),args=commandArr.join(' ')+(valueArg||'');document.execCommand(command,0,args);this.update();},get_current_range:function(){var sel=window.getSelection();if(sel.getRangeAt&&sel.rangeCount){return sel.getRangeAt(0);}},save_selection:function(){this.selected_range=this.get_current_range();},restore_selection:function(){var selection=window.getSelection();if(this.selected_range){selection.removeAllRanges();selection.addRange(this.selected_range);}},mark_selection:function(input,color){this.restore_selection();document.execCommand('hiliteColor',0,color||'transparent');this.save_selection();input.data(this.options.selection_marker,color);},});bsHTMLEditor=Class.extend({init:function(){var me=this;this.modal=bs_get_modal("<i class='icon-code'></i> Edit HTML",'<textarea class="form-control" \
   ...
   </textarea>');this.modal.addClass("frappe-ignore-click");this.modal.find(".btn-primary").removeClass("hide").html(__("Update")).on("click",function(){me._html=me.modal.find("textarea").val();$.each(me.editor.dataurls,function(key,val){me._html=replace_all(me._html,key,val);});var editor=me.editor.data("object");editor.set_input(me._html);editor.options.change&&editor.options.change(editor.clean_html());me.modal.modal("hide");});},show:function(editor){var me=this;this.editor=editor;this.modal.modal("show");var html=me.editor.html();this.editor.dataurls={}
html=html.replace(/<img\s*src=\s*["\'](data:[^,]*),([^"\']*)["\']/g,function(full,g1,g2){var key=g2.slice(0,5)+"..."+g2.slice(-5);me.editor.dataurls[key]=g1+","+g2;return'<img src="'+key+'"';});this.modal.find("textarea").val(html_beautify(html));}});bsLinkEditor=Class.extend({init:function(toolbar){var me=this;this.toolbar=toolbar;this.modal=bs_get_modal("<i class='icon-globe'></i> Insert Link",'<div class="form-group">\
...
me.modal.modal("hide");return false;});},show:function(){this.modal.find("input[type=text]").val("");this.modal.modal("show");}});bs_get_modal=frappe.get_modal;;